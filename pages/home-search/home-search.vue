		...
		<view class="home-list">
			<view v-if="is_history" class="label-box">
				...
				<view v-if="historyLists.length > 0" class="label-content">
					<view v-for="(item,index) in historyLists" :key="index" @click="openHistroy(item)" class="label-content__item">
						{{item.name}}
					</view>
				</view>
			...
			<list-scroll v-else class="list-scroll">
				...
				<view v-if="searchList.length>0">
					<list-card :item="item" v-for="(item,index) in searchList" :key="item._id" @click="setHistroy"></list-card>
				</view>
